...
def test_scanned_pdf_routes_to_expert(monkeypatch):
    ...
    monkeypatch.setattr(builders, "extract_pdf_pages",
                        lambda payload: [{"page": 1, "text": ""}, {"page": 2, "text": ""}])

    ni = builders.normalize_payload(b"%PDF-1.4 fake", modality="pdf")
...
def test_text_pdf_is_extracted(monkeypatch):
    ...
    ni = builders.normalize_payload(b"%PDF", modality="pdf")
...
def test_text_is_unicode_stabilized():
    ...
    ni = builders.normalize_payload(raw, modality="text")
